 ...
 *
 * Modified Nov 2023 by Greg Haerr to add support for compilation on macOS
 * and allocating stack at fixed address.
 */

#define debug               0
#define SUPPORT_VECTOR      XNU

//#define STACKADDR ((void *)(0x100000000 - STACKLEN))  /* stack from 4G */
//#define STACKADDR ((void *)(0x0C0000000 - STACKLEN))  /* stack from 3G */
...
#define DEBUG2(STR,VAR,STR2,VAR2)           \
  if (debug) {                              \
    char ibuf[32];                          \
    Utox(ibuf, VAR);                        \
    Print(os, 2, STR, ibuf, 0l);            \
    Utox(ibuf, VAR2);                       \
    Print(os, 2, " ", STR2, ibuf, "\n", 0l);\
  }
...
__attribute__((__noreturn__)) static void Spawn(int os, const char *exe, int fd,
...
#if 0   /* ELF segment overlap check */
  ...
  for (i = 0; i < e->e_phnum; ++i) {
    ...
    if (p[i].p_filesz) {
      ...
      DEBUG1("FILE  ", p[i].p_offset & -pagesz);
      DEBUG2("LOAD  ", rc, "SIZE ", size);
      ...
      DEBUG2("BSS   ", rc, "SIZE ", size);
      if (rc < 0) Pexit(os, exe, rc, "bss mmap");
    }
  }

#if 1   /* allocate stack at 32-bit fixed address and copy argv/envp */
  {
  rc = Mmap(STACKADDR, STACKLEN, PROT_READ|PROT_WRITE,
            MAP_FIXED|MAP_PRIVATE|MAP_ANONYMOUS, -1, 0, os);
  if (rc < 0) Pexit(os, exe, rc, "stack mmap");
  DEBUG1("STACK ", rc);
  int n;
  int bytes = 0;
  long *lp = sp + 1;            /* skip argc */
  while (*lp) bytes += StrLen((char *)*lp++) + 1;
  lp++;                         /* end argv */
  while (*lp) bytes += StrLen((char *)*lp++) + 1;
  lp += 2;                      /* end envp, null auxv */
  bytes += (char *)lp - (char *)sp;
  bytes = (bytes + 15) & ~15;
  char *p = (char *)(rc + STACKLEN);
  long *sp2 = (void *)(rc + STACKLEN - bytes);
  ...
  *sp2++ = 0;                   /* null aux */
  ...
  DEBUG1("ENTRY ", dynbase+e->e_entry);
...
EXTERN_C __attribute__((__noreturn__)) void Main(long di, long *sp, char dl) {
  ...
  long *sp2 = (long *)__builtin_alloca(n);
  if ((long)sp2 & 15) ++sp2;
  for (; n > 0; n -= pagesz) {
    ((char *)sp2)[n - 1] = 0;
  }
